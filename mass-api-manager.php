...
add_shortcode('mam_video', function ($attr, $content) {
    ...
    $video_id = $attr['id'];
    return '<div class="mam_video_container"><iframe class="mam_campaign_main_container" src="https://www.youtube.com/embed/' . $video_id . '"></iframe></div>';
});
...
add_shortcode('mam_tag', function ($attr, $content) {
    ...
    if (!empty($post_tags)) {
        ...
        return '<div class="mam_tag_container">' . trim($result, $separator) . '</div>';
    }
    return '';
});
...
function db_process()
{
    ...
    if ($wpdb->get_var('SHOW TABLES LIKE "' . $table_name . '"') != $table_name) {
        ...
        $sql = 'alter table ' . $table_name . ' modify column content text';
        require_once(ABSPATH . 'wp-admin/includes/upgrade.php');
        $wpdb->query($sql);
    }
...
[pinterest]',
    ...
    if ($wpdb->get_var('SHOW TABLES LIKE "' . $table_name . '"') != $table_name) {
        $sql = 'create table ' . $table_name . '(
            ...
            del_fl_flag int, 
            del_ll_flag int, 
            enable_related_keywords int, 
            enable_rss int, 
            enable_facebook int, 
            enable_twitter int, 
            enable_flickr int, 
            enable_pinterest int, 
            create_date timestamp default current_timestamp, 
            primary key (id)
        )';
        require_once(ABSPATH . 'wp-admin/includes/upgrade.php');
                                                ...
                                                <div class="mam_template_list list-group">
                                                    <?php
                                                        ...
                                                        <a class="mam_template_list_item list-group-item list-group-item-action <?php if ($i == 0) echo 'active';
                                                            ...
                                                            <?php if ($template->name != '[default]') { ?><i class="flaticon-tool_delete"></i><?php } ?>
                                                        </a>
                                                    <?php
                                                        }
                                                        ?>
            ...
            <div class="tab-pane container fade" id="settings">
                <div class="section" id="general-settings">
                    ...
                    <dl class="section-settings">
                        ...
                        <dt class="ff_mod_roles ff_hide4site">
                            <p class="desc">Select Enable/Disable.</p>
                        </dt>
                        <dd>
                            <div class="checkbox-row"><input type="checkbox" value="yep" name="" id="mam_enable_rss" <?php if ($setting->enable_rss == 1) echo "checked"; ?>>
                        ...
                        <dd>
                            <div class="checkbox-row"><input type="checkbox" value="yep" name="" id="mam_enable_facebook" <?php if ($setting->enable_facebook == 1) echo "checked"; ?>>
                                <label for="">facebook posts</label>
                            </div>
                        </dd>
                        ...
                        <dd>
                            <div class="checkbox-row"><input type="checkbox" value="yep" name="" id="mam_enable_twitter" <?php if ($setting->enable_twitter == 1) echo "checked"; ?>>
                                <label for="">twitter posts</label>
                            </div>
                        </dd>
                        ...
                        <dd>
                            <div class="checkbox-row"><input type="checkbox" value="yep" name="" id="mam_enable_flickr" <?php if ($setting->enable_flickr == 1) echo "checked"; ?>>
                                <label for="">flickr posts</label>
                            </div>
                        </dd>
                        ...
                        <dd>
                            <div class="checkbox-row"><input type="checkbox" value="yep" name="" id="mam_enable_pinterest" <?php if ($setting->enable_pinterest == 1) echo "checked"; ?>>
                                <label for="">pinterest posts</label>
                            </div>
                        </dd>
                    </dl>
